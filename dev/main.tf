...
module "cognito_pool" {
  ...
  cognito_domain          = "${var.cognito_domain}.${var.domain_name}"
  userpool_name           = "${local.env}-${var.userpool_name}"
  provider_name           = "${local.env}-${var.provider_name}"
  sp_metadata_url         = var.sp_metadata_url
  callback_urls           = ["https://fekgpt.com/api/auth/callback/cognito", "http://localhost:3000/api/auth/callback/cognito"]
  logout_urls             = ["https://fekgpt.com", "http://localhost:3000"]
